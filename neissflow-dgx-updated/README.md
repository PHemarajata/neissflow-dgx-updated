...
## Container Management (Important for Singularity Users)

If you're experiencing issues with containers being repeatedly pulled (especially shovill and snippy), we've implemented several solutions:
...
# Pre-pull all containers before running the pipeline
./bin/pull_containers.sh

# Then run your pipeline
nextflow run neissflow/main.nf -profile singularity [other options]
```

### Enhanced Singularity Profile
The pipeline now includes enhanced container management that:
- Prevents simultaneous container pulls
- Uses dedicated cache directories
- Implements better retry logic for container issues
- Reduces container-related workflow failures

For detailed troubleshooting and container management information, see [`docs/CONTAINER_MANAGEMENT.md`](docs/CONTAINER_MANAGEMENT.md).

## Disk Space Management (Important for Assembly)

Assembly processes (especially Shovill/SPAdes) require significant disk space. If you encounter "No space left on device" errors:
...
### Recommendations
- **Minimum 50GB** free space in work directory
- **30GB** free space in temp directory (`/tmp` or `$TMPDIR`)
- Monitor disk usage during long runs
- Use dedicated work directories: `nextflow run -w /path/to/large/work/dir`

For comprehensive disk space management guidance, see [`docs/DISK_SPACE_MANAGEMENT.md`](docs/DISK_SPACE_MANAGEMENT.md). 
